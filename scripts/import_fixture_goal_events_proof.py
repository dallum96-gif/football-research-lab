...
def player_bridge(season):
    # Authoritative team reconstruction: FPL club -> FRL persistent team code.
    fpl = {}
    for row in player_research._load_season_rows(season):
        ...
        fpl[(norm(name), str(team["persistent_team_code"]).strip())] = (str(element), name)
...
def main():
    ...
    fixtures = load(query_lab.FIXTURE_FILE)

    # Only inspect canonical fixtures for the seasons represented by the raw proof data.
    ...
    # Build the exact source-match targets from the raw evidence.
    ...
    for row in raw:
        ...
        fixture, (resolved_source_id, home, away) = resolved[(season, source_id)]
        scorer_name = row["source_scorer_name"].strip()
        scorer_team = norm(row["source_scorer_team"])
        ...
        if not identity:
            raise RuntimeError(f"No verified player bridge: {season}/{source_id}/{row['source_scorer_id']}/{scorer_name}/{scorer_team}")
        ...
        if not scorer_side:
            raise RuntimeError(f"Scorer team does not match fixture sides: {scorer_name}/{scorer_team}")
