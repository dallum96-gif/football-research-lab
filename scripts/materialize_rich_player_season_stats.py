...
def _materialize_season(
    ...
    if season in CURRENT_SEASONS:
        ...
        for row in (
            ...
            try:
                minutes = float(
                    row.get("minutesPlayed")
                    or row.get("minsPlayed")
                    or 0
                )
            except (
                TypeError,
        ...
        if not grouped:
            raise RuntimeError(
                "No current fixture-player "
                f"participants found for {season}"
            )
        ...
        for source_player_id, records in sorted(
            grouped.items(),
            ...
            rows.append(
                {
                    ...
                    # FPL player_code and PulseLive playerId
                    # are the shared Opta/PulseLive identity.
                    # Current local FPL freshness does not
                    # govern this source-layer projection.
                    "player_code":
                        source_player_id,
        ...
        if len(rows) != 387:
            raise RuntimeError(
                "Expected 387 current participants; "
                f"materialised {len(rows)}."
            )
    ...
    fields = set(
        player_match_stats.source_fields(
            season
        )
    )
    ...
    for player_code, identity in sorted(
        bridge.items(),
        key=lambda item: item[0],
    ):
        ...
        records = (
            player_match_stats
            .player_match_records_for_player(
                source_player_id,
                season,
            )
        )
    ...
    if not rows:
        raise RuntimeError(
            "No governed player-season projection "
            f"rows materialised for {season}"
        )
...
def materialize(
    ...
    historical_selected = any(
        season in CORE_SEASONS
    ...
    if historical_selected:
        ...
        if (
            pl_root is None
            or not pl_root.is_dir()
        ):
            raise RuntimeError(
                "Preserved Premier-League-Stats "
                f"root not found: {pl_root}"
            )

        player_match_stats.PL_ROOT = (
            pl_root
        )
    ...
    season_order = {season: index for index, season in enumerate(SUPPORTED_SEASONS)}
    rows.sort(
        key=lambda row: (
...
def main() -> int:
    ...
    parser.add_argument("--pl-root", type=Path, required=False)
    parser.add_argument(
        "--season",
        action="append",
        dest="seasons",
