...
PATTERN = re.compile(
    r'(?ms)^(?P<indent>\s*)season\s*=\s*st\.selectbox\(\s*["\']Season["\'].*?\n'
    r'\s*teams\s*=\s*\[r\.get\(["\']team["\']\) for r in _league_table\(season\).*?\n'
    r'\s*team\s*=\s*st\.selectbox\(\s*["\']Team["\'].*?\n'
)
...
NEW = '''    st.markdown(
    ...
    with st.container():
        st.markdown('<div class="frl-team-context-controls">', unsafe_allow_html=True)
...
if not match:
    raise RuntimeError(
        f"Expected Season/Team selector assignments not found in {TARGET}; refusing to patch."
    )
...
print("TARGET: team_research_ui_v7.py")
