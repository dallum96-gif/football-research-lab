...
def kit_svg(team: str) -> str:
    ...
    return f"""<svg width='52' height='64' viewBox='0 0 52 64' aria-label='{html.escape(team)} kit'><path d='M17 4 L5 12 L10 24 L17 20 L17 45 Q26 51 35 45 L35 20 L42 24 L47 12 L35 4 L31 8 L21 8 Z' fill='{body}' stroke='rgba(24,23,20,.16)' stroke-width='1.5'/><path d='M17 4 L21 8 L17 16 L10 20 L5 12 Z' fill='{sleeve}' opacity='.95'/><path d='M35 4 L31 8 L35 16 L42 20 L47 12 Z' fill='{sleeve}' opacity='.95'/><path d='M20 8 Q26 13 32 8' fill='none' stroke='{collar}' stroke-width='3'/><rect x='17' y='45' width='7' height='14' rx='2' fill='{sock}' opacity='.95'/><rect x='28' y='45' width='7' height='14' rx='2' fill='{sock}' opacity='.95'/></svg>"""

def token_markup(name: str, pos: str, role: str, x: float, y: float) -> str:
    fill = "#ffffff" if role == "gk" else "#fffdf8"
    accent = "#9aaa42" if role == "m" else "#e85d3f" if role == "a" else "#68645c"
    return f"""<div class='player-token' style='left:{x}%;top:{y}%'><div class='player-shirt' style='--accent:{accent};background:{fill}'><span>{html.escape(pos)}</span></div><div class='player-name'>{html.escape(name)}</div></div>"""
...
def formation_pitch(team: str) -> str:
    ...
    tokens = "".join(token_markup(name,pos,role,*positions[i]) for i,(name,pos,role) in enumerate(data["players"]))
    return f"""<div class='pitch-wrap'><div class='pitch'><div class='pitch-mid'></div><div class='pitch-circle'></div><div class='pitch-box top'></div><div class='pitch-box bottom'></div>{tokens}</div></div>"""

def timeline_icon(kind: str) -> str:
    return {"goal":"⚽","card":"▮","sub":"↕"}.get(kind,"·")

st.markdown("""<style>
.proto-kicker{color:var(--frl-accent);font-size:.62rem;font-weight:800;letter-spacing:.16em;text-transform:uppercase;margin-bottom:.45rem}.proto-title{color:var(--frl-text);font-size:1.1rem;font-weight:800;line-height:1.1;margin:0}.proto-meta{color:var(--frl-muted);font-size:.74rem;line-height:1.45}.fixture-head{display:grid;grid-template-columns:1fr 170px 1fr;gap:1.5rem;align-items:center;margin:1rem 0 1.5rem}.fixture-team{text-align:center}.fixture-kit{height:66px;display:flex;justify-content:center;align-items:center;margin-bottom:.25rem}.fixture-team-name{color:var(--frl-text);font-size:1.25rem;font-weight:850;letter-spacing:-.03em}.fixture-team-meta{color:var(--frl-muted-soft);font-size:.65rem;text-transform:uppercase;letter-spacing:.1em;margin-top:.25rem}.fixture-score{text-align:center}.fixture-scoreline{color:var(--frl-text);font-size:4.2rem;font-weight:900;letter-spacing:-.07em;line-height:.9}.fixture-status{color:var(--frl-muted);font-size:.67rem;font-weight:800;text-transform:uppercase;letter-spacing:.12em;margin-top:.5rem}.proto-rule{height:2px;background:var(--frl-text);opacity:.9;margin:1rem 0 1.3rem}.section-kicker{color:var(--frl-accent);font-size:.61rem;font-weight:800;letter-spacing:.15em;text-transform:uppercase;margin-bottom:.55rem}.timeline{border-top:1px solid var(--frl-border)}.event{display:grid;grid-template-columns:50px 28px 1fr 45px;gap:.65rem;align-items:center;padding:.58rem 0;border-bottom:1px solid var(--frl-border)}.event-minute{font-size:.67rem;color:var(--frl-muted-soft);font-weight:800;text-align:right}.event-icon{width:24px;height:24px;border-radius:50%;display:flex;align-items:center;justify-content:center;background:var(--frl-surface-raised);border:1px solid var(--frl-border);font-size:.65rem}.event-main{font-size:.75rem;color:var(--frl-text);font-weight:800}.event-copy{font-size:.68rem;color:var(--frl-muted);margin-top:.07rem}.event-side{font-size:.62rem;font-weight:900;letter-spacing:.05em;text-align:right;color:var(--frl-muted)}.event.goal .event-icon{background:rgba(154,170,66,.16);border-color:rgba(154,170,66,.35)}.event.card .event-icon{background:rgba(232,93,63,.10);border-color:rgba(232,93,63,.28)}.pitch-wrap{background:#eef0dd;border:1px solid rgba(24,23,20,.1);border-radius:17px;padding:.8rem;box-shadow:0 10px 35px rgba(24,23,20,.05)}.pitch{position:relative;height:500px;border-radius:12px;overflow:hidden;background:#dfe6c3;border:2px solid rgba(255,255,255,.75)}.pitch-mid{position:absolute;left:0;right:0;top:50%;border-top:2px solid rgba(255,255,255,.72)}.pitch-circle{position:absolute;left:50%;top:50%;width:92px;height:92px;margin:-46px;border:2px solid rgba(255,255,255,.72);border-radius:50%}.pitch-box{position:absolute;left:50%;transform:translateX(-50%);width:190px;height:94px;border:2px solid rgba(255,255,255,.72)}.pitch-box.top{top:0;border-top:0;border-radius:0 0 75px 75px}.pitch-box.bottom{bottom:0;border-bottom:0;border-radius:75px 75px 0 0}.player-token{position:absolute;transform:translate(-50%,-50%);width:84px;text-align:center;z-index:2}.player-shirt{width:28px;height:30px;margin:auto;border-radius:8px 8px 9px 9px;border:1px solid rgba(24,23,20,.15);position:relative;box-shadow:0 3px 9px rgba(24,23,20,.10)}.player-shirt:before,.player-shirt:after{content:'';position:absolute;top:2px;width:9px;height:14px;background:var(--accent);border-radius:3px}.player-shirt:before{left:-6px}.player-shirt:after{right:-6px}.player-shirt span{font-size:.48rem;font-weight:900;color:#37352f;position:absolute;left:0;right:0;top:9px}.player-name{font-size:.55rem;line-height:1.05;font-weight:800;color:#26251f;margin-top:.16rem;text-shadow:0 1px 0 rgba(255,255,255,.45)}.kit-inline{display:flex;align-items:center;gap:.7rem}.kit-inline svg{display:block;flex:none}.kit-copy strong{display:block;color:var(--frl-text);font-size:1.05rem;font-weight:850}.kit-copy span{display:block;color:var(--frl-muted-soft);font-size:.63rem;text-transform:uppercase;letter-spacing:.1em;margin-top:.15rem}
...
header_meta = f"{fixture['season']} · Matchweek {fixture['gameweek']} · {fixture['kickoff_time'][:10]} · Emirates Stadium"
st.markdown(f"<div class='proto-meta'>{html.escape(header_meta)}</div>", unsafe_allow_html=True)
...
with left:
    ...
    st.caption("The first read stays compact; deeper evidence is one layer away.")
    st.markdown("<div class='timeline'>",unsafe_allow_html=True)
    for minute,side,kind,title,copy in TIMELINE:
        st.markdown(f"<div class='event {kind}'><div class='event-minute'>{html.escape(minute)}</div><div class='event-icon'>{timeline_icon(kind)}</div><div><div class='event-main'>{html.escape(title)}</div><div class='event-copy'>{html.escape(copy)}</div></div><div class='event-side'>{side}</div></div>",unsafe_allow_html=True)
...
if stats.get("status")=="AVAILABLE":
    home_core=stats["home"]["core"]; away_core=stats["away"]["core"]
    labels=["Possession","Shots","Shots on target","Passes","Accurate passes","Corners","Tackles","Interceptions","Clearances","Yellow cards"]
    df=pd.DataFrame([{"Statistic":label,home:home_core.get(label,"—"),away:away_core.get(label,"—")} for label in labels])
