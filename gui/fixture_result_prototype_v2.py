...
LINEUPS = {
    "Arsenal": {
        ...
        "players": [
            ("Petr Cech", "GK", "gk"), ("Héctor Bellerín", "RB", "d"),
            ("Rob Holding", "CB", "d"), ("Calum Chambers", "CB", "d"),
            ("Nacho Monreal", "LB", "d"), ("Mohamed Elneny", "DM", "m"),
            ("Francis Coquelin", "DM", "m"), ("Theo Walcott", "RW", "a"),
            ("Aaron Ramsey", "AM", "m"), ("Alex Iwobi", "LW", "a"),
            ("Alexis Sánchez", "ST", "a"),
        ],
    ...
    "Liverpool": {
        ...
        "players": [
            ("Simon Mignolet", "GK", "gk"), ("Nathaniel Clyne", "RB", "d"),
            ("Dejan Lovren", "CB", "d"), ("Ragnar Klavan", "CB", "d"),
            ("Alberto Moreno", "LB", "d"), ("Jordan Henderson", "CM", "m"),
            ("Georginio Wijnaldum", "CM", "m"), ("Adam Lallana", "CM", "m"),
            ("Sadio Mané", "RW", "a"), ("Roberto Firmino", "CF", "a"),
...
@st.cache_data
def get_fixture(season, fixture_id):
...
def kit_svg(team):
    ...
    return f'''<svg width="52" height="64" viewBox="0 0 52 64" aria-label="{html.escape(team)} kit">
      <path d="M17 4 L5 12 L10 24 L17 20 L17 45 Q26 51 35 45 L35 20 L42 24 L47 12 L35 4 L31 8 L21 8 Z" fill="{body}" stroke="rgba(24,23,20,.16)" stroke-width="1.5"/>
      <path d="M17 4 L21 8 L17 16 L10 20 L5 12 Z" fill="{sleeve}"/>
      <path d="M35 4 L31 8 L35 16 L42 20 L47 12 Z" fill="{sleeve}"/>
      <path d="M20 8 Q26 13 32 8" fill="none" stroke="{collar}" stroke-width="3"/>
      <rect x="17" y="45" width="7" height="14" rx="2" fill="{sock}"/><rect x="28" y="45" width="7" height="14" rx="2" fill="{sock}"/>
    </svg>'''
...
home_score = fixture.get("home_score") if fixture.get("home_score") not in (None, "") else "—"
away_score = fixture.get("away_score") if fixture.get("away_score") not in (None, "") else "—"
...
with left:
    ...
    for minute, side, kind, title, copy in TIMELINE:
        minute_label = f"{int(minute)}'" if float(minute).is_integer() else "45+1'"
        st.markdown(f'<div class="event"><div class="event-minute">{minute_label}</div><div class="event-icon">{icon(kind)}</div><div><div class="event-main">{html.escape(title)}</div><div class="event-copy">{html.escape(copy)}</div></div><div class="event-side">{side}</div></div>', unsafe_allow_html=True)
...
if stats.get("status") == "AVAILABLE":
    ...
    labels = ["Possession","Shots","Shots on target","Passes","Accurate passes","Crosses","Corners","Tackles","Interceptions","Clearances","Yellow cards"]
    df = pd.DataFrame([{"Statistic": label, home: hc.get(label, "—"), away: ac.get(label, "—")} for label in labels])
...
with st.expander("Context & provenance", expanded=False):
    st.write({"Canonical fixture ID": fixture.get("fixture_id"), "Source match ID": stats.get("source_match_id"), "Canonical fixture source": detail.get("provenance", {}).get("canonical_source"), "Identity source": detail.get("provenance", {}).get("identity_source")})
