...
def _player_css():
    st.markdown(
        ...
        .frl-player-table-row {
            display: grid;
            grid-template-columns: minmax(180px, 1.8fr) 5.2rem 7.4rem 4.5rem 5rem 4.2rem 4.8rem 4.8rem 5.7rem 5.7rem;
            gap: .22rem;
            align-items: center;
            min-width: 760px;
        }
        ...
        .frl-player-hero-num { color: var(--frl-text); font-weight: 820; }
        .frl-player-highlight { color: var(--frl-accent); font-weight: 850; }
        .frl-player-positive { color: var(--frl-secondary); font-weight: 820; }
        .frl-player-muted { color: var(--frl-muted-soft); }
...
def render_player_research_ui():
    ...
    sort_cols = st.columns([1.4, .8], gap="medium")
    with sort_cols[0]:
    ...
    # ------------------------------------------------------------
    # League-table style player table
    # ------------------------------------------------------------

    table_cells = [
        "Player", "Club", "Pos", "Min", "G", "A", "xG", "xA", "G/90", "xG/90"
    ]

    row_html = [
        "<div class='frl-player-table'>",
        "<div class='frl-player-table-header'>",
        "<div>Player</div>",
        "<div>Club</div>",
        "<div>Pos</div>",
        ...
        "<div style='text-align:right'>xG</div>",
        "<div style='text-align:right'>xA</div>",
        "<div style='text-align:right'>G/90</div>",
        "<div style='text-align:right'>xG/90</div>",
        "</div>",
    ]
    ...
    for player in filtered:
        ...
        xg = fmt(player["xg"])
        xa = fmt(player["xa"])
        goals90 = fmt(player["goals_per_90"], 3)
        xg90 = fmt(player["xg_per_90"], 3)
        position_value = player.get("position") or "—"

        row_html.extend(
            [
                "<div class='frl-player-table-row'>",
                f"<div class='frl-player-table-cell frl-player-name'>{player['player_name']}</div>",
                f"<div class='frl-player-table-cell frl-player-club'>{clubs_text}</div>",
                f"<div class='frl-player-table-cell frl-player-pos'>{position_value}</div>",
                f"<div class='frl-player-table-cell frl-player-num frl-player-hero-num'>{int(player['minutes']):,}</div>",
                f"<div class='frl-player-table-cell frl-player-num frl-player-highlight'>{goals}</div>",
                f"<div class='frl-player-table-cell frl-player-num'>{assists}</div>",
                f"<div class='frl-player-table-cell frl-player-num'>{xg}</div>",
                f"<div class='frl-player-table-cell frl-player-num'>{xa}</div>",
                f"<div class='frl-player-table-cell frl-player-num'>{goals90}</div>",
                f"<div class='frl-player-table-cell frl-player-num'>{xg90}</div>",
                "</div>",
            ]
        )

    row_html.append(
        "<div style='margin-top:.7rem;color:var(--frl-muted-soft);font-size:.61rem;'>"
        "Goals are highlighted for quick scanning · use Advanced conditions for deeper research.</div>"
    )
    row_html.append("</div>")
