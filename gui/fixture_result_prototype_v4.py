...
def pitch(team: str) -> str:
    info = LINEUPS[team]
    # Correct football-facing left/right: RB/RW are on the viewer's right; LB/LW on the viewer's left.
...
def ratio(home, away):
    h=num(home) or 0.0; a=num(away) or 0.0; total=h+a
    return 50.0 if total<=0 else 100*h/total
...
try:
    ...
    st.error(f"Unable to load fixture: {exc}"); st.stop()
...
with left:
    ...
    for minute,side,kind,title,copy in TIMELINE:
        ...
        if kind=='goal':
            score = copy.split('·')[-1].strip() if '·' in copy else ''
            st.markdown(f"<div class='goal-event'><div class='event-minute'>{minute}</div><div class='event-icon'>{icon}</div><div><div class='event-main'>{html.escape(title)}</div><div class='event-copy'>{html.escape(copy.replace(' · '+score,'') if score else copy)}</div></div><div class='goal-score'>{html.escape(score)}</div></div>",unsafe_allow_html=True)
        else:
...
if stats.get("status")=="AVAILABLE":
    ...
    cards=[("Possession",hc.get("Possession"),ac.get("Possession"),"share of ball"),("Shots",hc.get("Shots"),ac.get("Shots"),"attempts"),("On target",hc.get("Shots on target"),ac.get("Shots on target"),"shots"),("Passes",hc.get("Passes"),ac.get("Passes"),"completed attempts")]
    st.markdown("<div class='stat-tiles'>"+"".join(f"<div class='stat-tile'><div class='label'>{html.escape(label)}</div><div class='value'>{pretty(h)} · {pretty(a)}</div><div class='note'>{home} · {away} · {note}</div></div>" for label,h,a,note in cards)+"</div>",unsafe_allow_html=True)
    st.markdown("<div class='viz-grid'>",unsafe_allow_html=True)
    st.markdown(f"<div class='viz-card'><div class='viz-title'>Territory & rhythm</div>"+ratio_bar("Possession",hc.get("Possession"),ac.get("Possession"),"%")+ratio_bar("Passes",hc.get("Passes"),ac.get("Passes"))+ratio_bar("Accurate passes",hc.get("Accurate passes"),ac.get("Accurate passes"))+"</div>",unsafe_allow_html=True)
    st.markdown(f"<div class='viz-card'><div class='viz-title'>Pressure & work</div>"+ratio_bar("Tackles",hc.get("Tackles"),ac.get("Tackles"))+ratio_bar("Interceptions",hc.get("Interceptions"),ac.get("Interceptions"))+ratio_bar("Clearances",hc.get("Clearances"),ac.get("Clearances"))+"</div>",unsafe_allow_html=True)
    xgh, xga = hc.get("Expected goals"), ac.get("Expected goals")
    if xgh is not None or xga is not None:
        st.markdown(f"<div class='viz-card' style='margin-top:.7rem'><div class='viz-title'>Chance quality</div>{ratio_bar('xG',xgh,xga)}<div class='insight'>{html.escape(home)} had {pretty(xgh)} expected goals; {html.escape(away)} had {pretty(xga)}. The result and chance profile can tell different stories.</div></div>",unsafe_allow_html=True)
    else:
        st.markdown(f"<div class='viz-card' style='margin-top:.7rem'><div class='viz-title'>The shape of the game</div><div class='insight'>The next layer can add shot quality, territory, chance creation and event-derived context without changing the compact visual language.</div></div>",unsafe_allow_html=True)
else:
    st.info("Historical match statistics are not available for this fixture.")
st.markdown("</div>",unsafe_allow_html=True)

with st.expander("Deeper evidence & provenance",expanded=False):
