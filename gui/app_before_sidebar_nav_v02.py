...
def render_player_research():
    ...
    st.caption(
        "Explore player performance and build Football Manager-style "
        "statistical filters from the underlying gameweek data."
    )
    ...
    if not seasons:
        st.error(
            "No player gameweek data was found."
        )
        return
    ...
    for i in range(int(filter_count)):
        stat_col, op_col, value_col = st.columns(
            [2.2, 1.4, 1.1]
        )
    ...
    st.caption(
        f"{len(filtered)} player(s) match the selected criteria"
    )
    ...
    with sort_col:
        sort_label = st.selectbox(
            "Sort by",
            sort_options,
            index=sort_options.index("Goals"),
            key="player_sort_stat",
        )
    ...
    def sort_value(player):
        ...
        if sort_label == "Club":
            return player["team_code"].casefold()

        if sort_label == "Appearances":
            return player["appearances"]

        value = player.get(
            stat_options[sort_label]
        )
        ...
        result_rows.append(
            {
                ...
                "xG": (
                    round(player["expected_goals"], 2)
                    if player["expected_goals"] is not None
                    else None
                ),
                "xA": (
                    round(player["expected_assists"], 2)
                    if player["expected_assists"] is not None
                    else None
                ),
                "Goals / 90": round(
                    player["goals_per_90"], 3
                ),
                "xG / 90": (
                    round(player["xg_per_90"], 3)
                    if player["xg_per_90"] is not None
                    else None
                ),
                "xA / 90": (
                    round(player["xa_per_90"], 3)
                    if player["xa_per_90"] is not None
                    else None
                ),
                "FPL": int(player["total_points"]),
                "BPS": int(player["bps"]),
            }
        )
    ...
    player_options = {
        player["player_name"]: player["player_code"]
        for player in filtered
    }
    ...
    selected = player_research.player_detail(
        season,
    ...
    if selected:
        ...
        st.markdown(
            f"### {selected['player_name']}"
        )

        st.caption(
            f"{selected['team_code']} Â· "
            f"{selected['position']} Â· "
            f"{season}"
        )
        ...
        metrics[2].metric(
            "Goals",
            int(selected["goals_scored"]),
        )
        metrics[3].metric(
            "Assists",
            int(selected["assists"]),
        )
        metrics[4].metric(
            "xG",
            (
                f"{selected['expected_goals']:.2f}"
                if selected["expected_goals"] is not None
                else "0"
            ),
        )
        metrics[5].metric(
            "xA",
            (
                f"{selected['expected_assists']:.2f}"
                if selected["expected_assists"] is not None
                else "0"
            ),
        )
        ...
        for record in selected["_records"]:
            def safe_int(value):
                try:
