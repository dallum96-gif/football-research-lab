...
export default async function TeamStatsPage({
  ...
  const requestedTeam =
    ...
    governedTeams.some(
      (team) =>
        team.persistent_team_code === query.team
    )
      ? query.team
  ...
  const selectedTeam = governedTeams.find(
    (team) =>
      team.persistent_team_code === requestedTeam
  );
  ...
  const strongest =
    overview?.metrics.reduce<Metric | null>(
      (best, metric) =>
        !best ||
        metric.percentile > best.percentile
          ? metric
          : best,
      null
    ) ?? null;
  ...
  const weakest =
    overview?.metrics.reduce<Metric | null>(
      (worst, metric) =>
        !worst ||
        metric.percentile < worst.percentile
          ? metric
          : worst,
      null
    ) ?? null;

  const home = overview?.splits.find(
    (split) => split.label === "Home"
  );

  const away = overview?.splits.find(
    (split) => split.label === "Away"
  );
    ...
    <AppShell>
      <div className={styles.page}>
        ...
        <nav
          ...
          aria-label="Team Stats sections"
        >
          {tabs.map((tab, index) => (
            <span
              key={tab}
        ...
        {overview ? (
            ...
            <section className={styles.analysisGrid}>
              <article className={styles.trendPanel}>
                ...
                >
                  ...
                  <div
                    className={styles.trendReadout}
                  >
                    <span>Latest five</span>
                    <strong>
                      {latestRolling !== null
                        ? latestRolling.toFixed(2)
                        : "—"}
                    </strong>
                    <small>
                      Season{" "}
              ...
              <article className={styles.venuePanel}>
                  ...
                  (split) =>
                    split && (
                      ...
                      >
                        ...
                        <div
                          className={styles.splitName}
                        >
                          <strong>
                            {split.label}
                          </strong>
                          <span>
                            {split.matches} matches
                          </span>
                        </div>

                        <div
                          className={styles.splitStat}
                        >
                          <strong>
                            {split.points_per_match?.toFixed(
                              2
                            ) ?? "—"}
                          </strong>
                          <span>PPG</span>
                        ...
                        >
                          <strong>
                            {split.goals_for_per_match?.toFixed(
                              2
                            ) ?? "—"}
                          </strong>
                          <span>GF</span>
                        ...
                        >
                          <strong>
                            {split.goals_against_per_match?.toFixed(
                              2
                            ) ?? "—"}
                          </strong>
                          <span>GA</span>
                ...
                <div className={styles.venueNote}>
                  {home?.points_per_match != null &&
                  away?.points_per_match != null ? (
                    <>
                      <span>
                        Venue effect
                      </span>
                      <strong>
                        {Math.abs(
                          home.points_per_match -
                            away.points_per_match
                        ).toFixed(2)}{" "}
                        PPG
                      </strong>
                      <p>
              ...
              >
                ...
                >
                  ...
                  <div>
                    <span>xG / match</span>
                    <strong>
                      {overview.expected_goals_per_match !==
                      null
                        ? overview.expected_goals_per_match.toFixed(
                            2
                          )
                        : "—"}
                    </strong>
                ...
                <footer>
                  League context, not a prediction
                  model.
                </footer>
              </article>
            </section>
          ...
          <div className={styles.empty}>
            Team Stats data is unavailable for this
            selection.
          </div>
        )}
      </div>
