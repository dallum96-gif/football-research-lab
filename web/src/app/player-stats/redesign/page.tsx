...
type SimilarPlayer = PlayerOption & {
  similarity: number;
...
function formatMetric(metric: { unit: string }, value: number | null) {
...
function polarPoint(
  ...
  return `${cx + Math.cos(angle) * radius * ratio},${
    cy + Math.sin(angle) * radius * ratio
  }`;
}

function radarPolygon(values: number[], radius = 83, cx = 122, cy = 104) {
  return values
    .map((value, index) =>
      polarPoint(index, values.length, value, radius, cx, cy)
    )
    .join(" ");
}
...
function Fingerprint({ radar }: { radar: RadarResult }) {
  ...
  return (
    <svg
      ...
      viewBox="0 0 244 218"
      role="img"
      aria-label="Position percentile fingerprint"
    >
      {rings.map((ring) => (
        <polygon
          key={ring}
          points={radarPolygon(Array(axes.length).fill(ring))}
          className={styles.radarRing}
        />
      ))}
      {axes.map((axis, index) => {
        const outer = polarPoint(index, axes.length, 100, 83, 122, 104)
          .split(",")
          .map(Number);
        const label = polarPoint(index, axes.length, 124, 83, 122, 104)
          .split(",")
          .map(Number);
        return (
          <g key={axis.key}>
            ...
            >
              {percentileLabel(axis.percentile)}
            </text>
          </g>
        );
      })}
      ...
      {values.map((value, index) => {
        const [x, y] = polarPoint(index, values.length, value, 83, 122, 104)
          .split(",")
          .map(Number);
        return (
          <circle
...
function MetricRows({
  ...
  const visible = typeof limit === "number" ? metrics.slice(0, limit) : metrics;
  return (
...
function ScatterPlot({
  ...
  if (!xMetric || !yMetric) {
    return (
      <div className={styles.pendingPanel}>
        Relationship plot unavailable for this cohort.
      </div>
    );
  }

  const yByPlayer = new Map(
    yMetric.entries.map((entry) => [entry.player_code, entry])
  );
  const points = xMetric.entries
    .map((entry) => ({ x: entry, y: yByPlayer.get(entry.player_code) }))
    .filter(
      (pair): pair is { x: RankingEntry; y: RankingEntry } =>
        pair.x.value != null && pair.y?.value != null
    );

  if (points.length < 2) {
    return (
      <div className={styles.pendingPanel}>
        Not enough observed evidence for this relationship plot.
      </div>
    );
  }
  ...
  const xScale = (value: number) =>
    pad +
    ((value - xMin) / Math.max(0.0001, xMax - xMin)) * (width - pad * 2);
  const yScale = (value: number) =>
    height -
    pad -
    ((value - yMin) / Math.max(0.0001, yMax - yMin)) *
      (height - pad * 2);
  ...
  return (
    <div className={styles.scatterWrap}>
      <svg
        ...
        role="img"
        aria-label={`${xMetric.label} against ${yMetric.label}`}
      >
        {[0.25, 0.5, 0.75].map((ratio) => (
          <g key={ratio}>
            <line
              ...
              className={styles.scatterGrid}
            />
            <line
              y1={pad}
              y2={height - pad}
              x1={pad + (width - pad * 2) * ratio}
              x2={pad + (width - pad * 2) * ratio}
              className={styles.scatterGrid}
            />
          </g>
        ))}
        ...
        {points.map(({ x, y }) => {
          ...
          const compared = Boolean(compareCode && x.player_code === compareCode);
          return (
            <circle
              key={x.player_code}
              ...
              className={
                highlighted
                  ? styles.scatterPrimary
                  : compared
                    ? styles.scatterCompare
                    : styles.scatterPoint
              }
            >
              <title>{`${x.player_name}: ${xMetric.label} ${formatMetric(
                xMetric,
                x.value
              )}, ${yMetric.label} ${formatMetric(yMetric, y.value)}`}</title>
            </circle>
          );
        })}
        <text
      ...
      <div className={styles.scatterLegend}>
        <span>
          <i className={styles.legendPrimary} /> Selected player
        </span>
        {compareCode ? (
          <span>
            <i className={styles.legendCompare} /> Comparison player
          </span>
        ) : null}
        <span>
          <i /> Same-position cohort
        </span>
      </div>
    </div>
  );
...
function similarPlayers(
  ...
  const rankingByKey = new Map(rankings.metrics.map((metric) => [metric.key, metric]));
  const selectedPercentiles = new Map(
    metrics
      .filter((metric) => metric.percentile != null)
      .map((metric) => [metric.key, metric.percentile as number])
  );

  return players
    .filter(
      (player) =>
        player.player_code !== selected.player_code &&
        player.position === selected.position &&
        player.minutes >= qualificationMinutes
    )
    .map((player) => {
      const gaps: number[] = [];
      metricKeys.forEach((key) => {
        const selectedPercentile = selectedPercentiles.get(key);
        const rankingMetric = rankingByKey.get(key);
        const candidate = rankingMetric?.entries.find(
          (entry) => entry.player_code === player.player_code
        );
        if (selectedPercentile != null && candidate?.percentile != null) {
          gaps.push(Math.abs(selectedPercentile - candidate.percentile));
        }
      });
      const similarity =
        gaps.length >= 4
          ? Math.max(
              ...
              Math.min(
                100,
                100 - gaps.reduce((sum, gap) => sum + gap, 0) / gaps.length
              )
            )
          : -1;
      return { ...player, similarity, comparedMetrics: gaps.length };
    })
    .filter((player) => player.similarity >= 0)
    .sort(
      (a, b) =>
...
export default async function PlayerStatsRedesignPage({
  searchParams,
}: {
  ...
  if (!season) {
    return (
      <AppShell>
        <div className={styles.empty}>No governed Player Stats season is available.</div>
      </AppShell>
    );
  }
  ...
  const requested =
    query.player &&
    eligiblePlayers.find((player) => player.player_code === query.player);
  const fallback =
    eligiblePlayers.find((player) => player.player_name === "Declan Rice") ??
    eligiblePlayers[0];
    ...
    return (
      <AppShell>
        <div className={styles.empty}>
          No player with recorded minutes is available for {season}.
        </div>
      </AppShell>
    );
  }
  ...
  const [stats, profile, radar, rankings] = await Promise.all([
    getJson<PlayerStatsResult>(
      `/api/v1/player-stats/${encodeURIComponent(season)}/${encodeURIComponent(
        playerCode
      )}`
    ),
    ...
    getJson<PlayerRankingsResult>(
      `/api/v1/player-stats/${encodeURIComponent(
        season
      )}/rankings/${encodeURIComponent(selected.position)}`
    ),
  ]);
  ...
  if (!stats || !profile || !radar || !rankings) {
    return (
      <AppShell>
        <div className={styles.empty}>
          The governed Player Stats workspace could not be assembled for this player.
        </div>
      </AppShell>
    );
  }
  ...
  const activeView =
    query.view === "explore" || query.view === "compare"
      ? query.view
      : "overview";
  const availableFamilies = Array.from(
    new Set(stats.metrics.map((metric) => metric.family))
  ).filter((family) => family !== "overview");
  const activeFamily =
    query.family && availableFamilies.includes(query.family)
      ? query.family
      : availableFamilies[0] ?? "shooting";
  ...
  const keyMetrics = preferredKeys
    ...
    .filter(
      (metric): metric is PlayerStatsMetric =>
        Boolean(metric && metric.percentile != null)
    )
    .slice(0, 12);
  ...
  const headlineAxes = [...radar.axes]
    ...
    .slice(0, 4);
  const rankedAxes = [...radar.axes]
    .filter((axis) => axis.percentile != null)
    .sort((a, b) => (b.percentile ?? 0) - (a.percentile ?? 0));
  const strongest = rankedAxes.slice(0, 2);
  const lower = [...rankedAxes]
  ...
  const samePositionPlayers = eligiblePlayers
    .filter(
      (player) =>
        player.position === selected.position && player.player_code !== playerCode
    )
    .sort(
      (a, b) =>
        b.minutes - a.minutes || a.player_name.localeCompare(b.player_name)
    );
  const requestedCompare =
    query.compare &&
    samePositionPlayers.find((player) => player.player_code === query.compare);
  const comparePlayer: PlayerOption | undefined =
    requestedCompare ||
    (activeView === "compare" ? samePositionPlayers[0] : undefined);
  const compareStats = comparePlayer
    ? await getJson<PlayerStatsResult>(
        `/api/v1/player-stats/${encodeURIComponent(
          season
        )}/${encodeURIComponent(comparePlayer.player_code)}`
      )
    : null;

  const compareMetricMap = new Map(
    (compareStats?.metrics ?? []).map((metric) => [metric.key, metric])
  );
  const familyMetrics = stats.metrics.filter(
    (metric) => metric.family === activeFamily && metric.percentile != null
  );
  const [scatterX, scatterY] = SCATTER_KEYS_BY_POSITION[selected.position] ?? [
    stats.overview_keys[0],
    stats.overview_keys[1],
  ];
        ...
        <div className={styles.inner}>
          <header className={styles.hero}>
            <section className={styles.heroCopy}>
              ...
              <p className={styles.identityLine}>
                {club} <span>·</span> {profile.position} <span>·</span>{" "}
                {biography.nationality ?? profile.competition}
              </p>
              <p className={styles.heroSummary}>
                {radar.summary ||
                  `Current-season analytical profile for ${profile.player_name}.`}
              </p>
              <div className={styles.heroActions}>
                ...
                <Link
                  ...
                  href={viewHref(season, playerCode, "compare")}
                >
                  Add to Compare +
                </Link>
              </div>
            ...
            <div className={styles.portraitStage}>
              <PlayerStatsV2Portrait
                playerCode={profile.portrait_player_code ?? playerCode}
            ...
            <aside className={styles.identityRail}>
              <div className={styles.railHeading}>
                <span>
                  {positionLabel(profile.position)} · {biography.shirt_number ?? "—"}
                </span>
              ...
              <dl>
                <div>
                  <dt>Born</dt>
                  <dd>{formatDate(biography.birth_date)}</dd>
                </div>
                <div>
                  <dt>Nationality</dt>
                  <dd>{biography.nationality ?? "—"}</dd>
                </div>
                <div>
                  <dt>Height</dt>
                  <dd>
                    {biography.height_cm
                      ? `${Math.round(biography.height_cm)} cm`
                      : "—"}
                  </dd>
                </div>
                <div>
                  <dt>Preferred foot</dt>
                  <dd>{biography.preferred_foot ?? "—"}</dd>
                </div>
                <div>
                  <dt>Joined</dt>
                  <dd>{formatDate(biography.join_date)}</dd>
                </div>
              </dl>
              <div className={styles.crestLine}>
                <TeamCrest teamName={club} size={44} />
                <span>{club}</span>
              </div>
            </aside>
          </header>
          ...
          <section className={styles.controlBar}>
            <nav className={styles.modeTabs} aria-label="Player Stats modes">
              {(["overview", "explore", "compare"] as const).map((view) => (
                <Link
                  key={view}
                  className={
                    activeView === view ? styles.modeActive : styles.modeLink
                  }
                  href={viewHref(season, playerCode, view)}
                >
                  {view[0].toUpperCase() + view.slice(1)}
                </Link>
              ))}
            </nav>
            <PlayerStatsV2Controls
              seasons={seasons}
            ...
            <div className={styles.sampleState} data-status={sampleStatus}>
              <i />
              <span>
                {radar.sample_minutes ?? profile.minutes} minutes ·{" "}
                {sampleStatus.toLowerCase().replaceAll("_", " ")}
              </span>
            </div>
          </section>
              ...
              <section className={styles.headlineGrid}>
                {headlineAxes.map((axis, index) => (
                  ...
                  >
                    ...
                    <div>
                      <strong>{percentileLabel(axis.percentile)}</strong>
                      <span className={styles.headlineBar}>
                        <i style={{ width: `${axis.percentile ?? 0}%` }} />
                      </span>
                    </div>
              ...
              <section className={styles.overviewGrid}>
                <article className={styles.fingerprintPanel}>
                  <header className={styles.panelHeading}>
                    <div>
                      <p>Player fingerprint</p>
                      <h2>Position percentile profile</h2>
                    </div>
                    <span>
                ...
                <article className={styles.metricsPanel}>
                  ...
                  <MetricRows metrics={keyMetrics} limit={10} />
                </article>
                ...
                <aside className={styles.contextStack}>
                  <section className={styles.contextPanel}>
                    ...
                    <dl>
                      ...
                      <div>
                        <dt>Qualification</dt>
                        <dd>{qualificationMinutes ? `${qualificationMinutes} min` : "—"}</dd>
                      </div>
                      <div>
                        <dt>Progress</dt>
                        <dd>{qualificationMinutes ? `${qualificationProgress}%` : "—"}</dd>
                      </div>
                    </dl>
                  </section>
                  ...
                  <section className={styles.signalPanel}>
                    <header>
                      <p>Profile signals</p>
                      <h2>Relative markers</h2>
                    </header>
                    <div className={styles.signalColumns}>
                      <div>
                        <h3>Higher</h3>
                        {strongest.map((axis) => (
                          <div className={styles.signalRow} key={`high-${axis.key}`}>
                      ...
                      <div>
                        <h3>Lower</h3>
                        {lower.map((axis) => (
                          <div className={styles.signalRow} key={`low-${axis.key}`}>
                            <span>{axis.label}</span>
                            <strong>{percentileLabel(axis.percentile)}</strong>
                          </div>
                        ))}
                  ...
                  <section className={styles.similarPanel}>
                    <header>
                      <div>
                        <p>Similar players</p>
                        <h2>Style proximity</h2>
                      </div>
                      <Link href={viewHref(season, playerCode, "compare")}>
                        Compare →
                      </Link>
                    </header>
                        ...
                        similar.map((player) => (
                          <Link
                            ...
                            href={`${viewHref(
                              ...
                              playerCode,
                              "compare"
                            )}&compare=${encodeURIComponent(player.player_code)}`}
                          >
                            <TeamCrest
                              teamName={player.clubs[0] ?? "Club unavailable"}
                              size={24}
                            />
                            <span>
                              <strong>{player.player_name}</strong>
                              <small>{player.clubs[0] ?? "Club unavailable"}</small>
                            </span>
                            <b>{Math.round(player.similarity)}%</b>
                          </Link>
                        ))
                      ) : (
                        <small className={styles.similarPending}>
                          Not enough qualified overlapping metric evidence yet.
                        </small>
                      )}
                    </div>
                  </section>
          ...
          {activeView === "explore" ? (
            <main className={styles.workspace}>
              <section className={styles.exploreHeader}>
                <div>
                  <p className={styles.eyebrow}>Explore evidence</p>
                  <h2>Every governed Player Stats family</h2>
                </div>
                <nav aria-label="Metric families">
                  {availableFamilies.map((family) => (
                    <Link
                      ...
                      href={viewHref(season, playerCode, "explore", family)}
                    >
              ...
              <section className={styles.exploreLedger}>
                <header className={styles.panelHeading}>
                  <div>
                    <p>{FAMILY_LABELS[activeFamily] ?? activeFamily}</p>
                    <h2>Metric ledger</h2>
                  </div>
                  <span>{familyMetrics.length} observed metrics</span>
                </header>
                <div className={styles.scrollLedger}>
                  <MetricRows metrics={familyMetrics} />
                </div>
              </section>
              <section className={styles.methodPanel}>
                <strong>Comparison population</strong>
                <p>{stats.cohort.description}.</p>
              </section>
            </main>
          ) : null}
            ...
            <main className={styles.workspace}>
              <section className={styles.compareHeader}>
                <div>
                  ...
                  <h2>
                    {profile.player_name} against another{" "}
              ...
              <section className={styles.compareGrid}>
                <article className={styles.scatterPanel}>
                  <header className={styles.panelHeading}>
                    <div>
                      <p>Relationship plot</p>
                      <h2>
                        {rankings.metrics.find((metric) => metric.key === scatterX)
                          ?.label ?? scatterX}{" "}
                        ×{" "}
                        {rankings.metrics.find((metric) => metric.key === scatterY)
                          ?.label ?? scatterY}
                      </h2>
                    </div>
                    <span>{rankings.population_size} players</span>
                  </header>
                  <ScatterPlot
                    rankings={rankings}
                    xKey={scatterX}
                    yKey={scatterY}
                    playerCode={playerCode}
                    compareCode={comparePlayer?.player_code}
                  />
                </article>
                <article className={styles.directComparePanel}>
                  <header className={styles.panelHeading}>
                    <div>
                      ...
                      <h2>
                        {comparePlayer
                          ? `${profile.player_name} vs ${comparePlayer.player_name}`
                          : profile.player_name}
                      </h2>
                    </div>
                  </header>
                  {comparePlayer && compareStats ? (
                    <div className={styles.compareTable}>
                      <div className={styles.compareTableHead}>
                        ...
                        <strong>
                          {comparePlayer.player_name.split(" ").slice(-1)[0]}
                        </strong>
                      </div>
                      {keyMetrics.slice(0, 8).map((metric) => {
                        const other = compareMetricMap.get(metric.key);
                        return (
                          <div className={styles.compareTableRow} key={metric.key}>
                            <span>{metric.label}</span>
                            <strong>
                            ...
                            <strong>
                              {other ? formatMetric(other, other.value) : "—"}{" "}
                              <small>
                                {other ? percentileLabel(other.percentile) : "—"}
                              </small>
                            </strong>
                          </div>
                        );
                    ...
                    <div className={styles.pendingPanel}>
                      Select a comparison player to open the direct evidence view.
                    </div>
                  )}
                </article>
              </section>
