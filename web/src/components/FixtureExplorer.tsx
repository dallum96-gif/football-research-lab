...
export function FixtureExplorer() {
  ...
  const orderedSeasons = useMemo(() => [...seasons].sort((a, b) => seasonKey(a) - seasonKey(b)), [seasons]);
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    Promise.allSettled(selectedSeasons.map((selected) => fetchFixtureResearchResult(selected, team, controller.signal)))
      .then((results) => {
        const successful = results.flatMap((result) => result.status === "fulfilled" ? [result.value] : []);
        const failedCount = results.length - successful.length;
        if (!successful.length) throw new Error("No trusted fixture results were available for the selected period.");
        ...
        setResultId(view === "single" ? successful[0]?.result_id ?? "" : `fixtures:${scopeStart}:${scopeEnd}:${team}:all:all:all`);
        setDescription(view === "single" ? successful[0]?.description ?? "" : `${team} fixtures across ${scopeStart} to ${scopeEnd}, composed from season-specific trusted fixture research results.`);
        setPopulationLabel(view === "single" ? successful[0]?.population.label ?? "" : `${team} Premier League fixtures across ${successful.length} available seasons in the selected period`);
  ...
  function updateExplore(key: "view" | "start" | "end", value: string) {
    ...
    if (key === "view") {
      ...
      if (seasonKey(nextStart) > seasonKey(nextEnd)) {
        if (key === "start") next.set("end", value); else next.set("start", value);
      }
    ...
    <>
      <section className="frl-page-heading">
        <div className="frl-heading-copy">
          ...
          <div className="frl-heading-row">
            <div className="frl-team-title-control">
              <button type="button" className={`frl-team-title-button${teamMenuOpen ? " is-open" : ""}`} aria-label={`Change team from ${team}`} aria-expanded={teamMenuOpen} disabled={contextLoading || !!error} onClick={() => setTeamMenuOpen((open) => !open)}>
              ...
              {teamMenuOpen && (
                <div className="frl-team-menu" role="menu" aria-label="Choose team">
                  {teamOptions.map((value) => <button key={value} type="button" role="menuitem" className={value === team ? "is-active" : ""} onClick={() => updateContext("team", value)}>{value}</button>)}
                </div>
              )}
            </div>
          </div>
        ...
        <div className="frl-context-actions">
          ...
          <div className="frl-record-chip" aria-label="Fixture record"><strong>{filtered.length}</strong><span>matches</span><span className="frl-record-positive">{record.wins} W</span><span>{record.draws} D</span><span className="frl-record-negative">{record.losses} L</span></div>
        </div>
      </section>

      <div className="frl-rule" />

      <section className="frl-filter-bar" aria-label="Fixture exploration" style={{ display: "flex", flexWrap: "wrap", alignItems: "flex-end", gap: "1rem" }}>
        <div className="frl-filter-heading"><span>Explore fixtures</span><strong>{view === "multi" ? `${effectiveStartSeason} to ${effectiveEndSeason}` : hasFilters ? "Filtered view" : "All fixtures"}</strong></div>

        <div className="frl-context-control"><span>View</span><select value={view} onChange={(event) => updateExplore("view", event.target.value)} aria-label="Fixture view"><option value="single">Single season</option><option value="multi">Multiple seasons</option></select><span className="frl-context-chevron" aria-hidden="true">⌄</span></div>
        {view === "multi" && <>
          <div className="frl-context-control"><span>From</span><select value={effectiveStartSeason} onChange={(event) => updateExplore("start", event.target.value)} aria-label="Starting season">{orderedSeasons.map((value) => <option key={value}>{value}</option>)}</select><span className="frl-context-chevron" aria-hidden="true">⌄</span></div>
          <div className="frl-context-control"><span>To</span><select value={effectiveEndSeason} onChange={(event) => updateExplore("end", event.target.value)} aria-label="Ending season">{orderedSeasons.map((value) => <option key={value}>{value}</option>)}</select><span className="frl-context-chevron" aria-hidden="true">⌄</span></div>
        </>}
        <div className="frl-context-control"><span>Opponent</span><select value={opponent} onChange={(event) => updateFilter("opponent", event.target.value)} disabled={loading || !!error} aria-label="Opponent"><option value="">All opponents</option>{opponents.map((value) => <option key={value}>{value}</option>)}</select><span className="frl-context-chevron" aria-hidden="true">⌄</span></div>
        <div className="frl-context-control"><span>Venue</span><select value={venue} onChange={(event) => updateFilter("venue", event.target.value)} disabled={loading || !!error} aria-label="Venue"><option value="">Home + Away</option><option>Home</option><option>Away</option></select><span className="frl-context-chevron" aria-hidden="true">⌄</span></div>
        <div className="frl-context-control"><span>Result</span><select value={resultFilter} onChange={(event) => updateFilter("result", event.target.value)} disabled={loading || !!error} aria-label="Result"><option value="">All results</option><option>W</option><option>D</option><option>L</option><option>UNPLAYED</option></select><span className="frl-context-chevron" aria-hidden="true">⌄</span></div>
        <div className="frl-filter-summary" style={{ marginLeft: "auto" }}><span>{loading ? "Loading research result…" : `${filtered.length} of ${rows.length} fixtures`}</span>{hasFilters && <button type="button" onClick={clearFilters}>Clear</button>}</div>
      </section>

      {error ? <div className="frl-empty-state"><strong>Fixture research result unavailable.</strong><div>{error}</div><small>The page has failed closed rather than falling back to fabricated or stale fixture data.</small></div> : loading ? <div className="frl-empty-state">Loading the validated fixture research result…</div> : grouped.length === 0 ? <div className="frl-empty-state"><strong>No fixtures match those filters.</strong><button type="button" onClick={clearFilters}>Clear filters</button></div> : (
        <section className="frl-fixture-list" aria-label="Fixture results">
          {grouped.map(([group, groupRows]) => <div className="frl-fixture-month" key={group}>
            <div className="frl-month-heading"><span />{group}</div>
            <div className="frl-fixture-header"><span>Date</span><span>Opponent</span><span>Venue</span><span>Score</span><span>Result</span></div>
            {groupRows.map((row) => <Link className="frl-fixture-row" href={`/fixtures/${row.season}/${row.fixtureId}`} key={`${row.season}-${row.fixtureId}`}>
              <span className="frl-fixture-date">{row.date}{row.gameweek ? <small>GW {row.gameweek}</small> : null}</span>
      ...
      <div className="frl-research-note"><strong>Research result.</strong> {description}<br /><span>{populationLabel}</span><br /><span>Result: {resultId}</span><br /><span>Provenance: {provenance}</span></div>
    </>
  );
}
