...
Fixture Explorer is the first real Next.js research workspace and is now backed by the real Python research/query seam through FastAPI.

Validated boundary:

```text
Next Fixture Explorer
    ↓
FastAPI
    ↓
query_api.fixtures()
    ↓
trusted FRL fixture data
```
...
The fixture Research Result preserves `(season, fixture_id)` canonical references and includes population, scope, provenance, methodology and limitations. Historical information-availability snapshots remain explicitly unasserted by this endpoint.

Current Fixture Explorer capabilities:

- real team/season context;
- URL-backed team and season state;
- canonical fixture deep-links;
- opponent/venue/result filters;
- chronological grouped fixture rows;
- W/D/L summary;
- fail-closed loading/error behaviour;
- research-result provenance display.

The current approved selector pattern is now a migration design precedent:

```text
Page context
  → integrated into the heading
  → transparent / typographic / subtle underline / discrete chevron

Research exploration
  → integrated into a purposeful section
  → uses the same selector language
  → avoids generic boxed widgets
```

The team selector is represented by the primary entity heading plus a discrete chevron; the heading itself has no surrounding box. Season remains a quiet heading-level selector. The lower controls now live in an `Explore / Fixture view` section.

The `Explore` section is deliberately the home for richer fixture exploration. The intended progression is:

1. opponent selection — implemented;
2. venue/result filtering — implemented;
3. single-season vs multi-season viewing — next;
4. opponent comparison across a selected period — next-stage extension;
5. compact time-range/comparison control — later;
6. deeper cross-time analytical interpretation — Team Research / Research Result surfaces.

Do not overload or redesign the clean header simply to add these analytical controls. Header = context. Explore section = analysis controls.
...
The foundation has now progressed into the first real workspace checkpoint described in Phase 2 above. The remaining work is incremental workspace migration, real analytical expansion and deeper visual research capabilities.
