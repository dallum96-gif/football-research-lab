...
def _aggregate(
    ...
    # Additive player-match evidence is layered on after the established
    # FPL aggregation. It never changes the existing aggregation contract.
    player = player_research_player_match.enrich_aggregate(player)

    # The existing Passing UI already consumes these field names. For a
    # verified player-match identity, use the audited external source for
    # pass totals, completed passes, key passes and big chances created.
    # Leave the values unavailable rather than converting absent evidence to 0.
    if player.get("player_match_identity_status") == "VERIFIED":
        source_passes = player.get("player_match_passes")
        source_accurate = player.get("player_match_accurate_passes")
        source_key = player.get("player_match_key_passes")
        source_big = player.get("player_match_big_chances_created")

        if source_passes is not None:
            player["attempted_passes"] = source_passes
        if source_accurate is not None:
            player["completed_passes"] = source_accurate
        if source_key is not None:
            player["key_passes"] = source_key
        if source_big is not None:
            player["big_chances_created"] = source_big
