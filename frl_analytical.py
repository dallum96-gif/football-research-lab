...
def _source_signature() -> tuple[int, int]:
    ...
    return FIXTURE_FILE.stat().st_mtime_ns, TEAM_REGISTRY.stat().st_mtime_ns


@lru_cache(maxsize=4)
def _cached_connection(
    fixture_mtime_ns: int,
    registry_mtime_ns: int,
) -> duckdb.DuckDBPyConnection:
    del fixture_mtime_ns, registry_mtime_ns
    con = duckdb.connect(database=":memory:")
    con.execute(
        f"CREATE VIEW fixtures AS SELECT * FROM read_csv_auto('{_escape(FIXTURE_FILE)}', SAMPLE_SIZE=-1, HEADER=TRUE)"
    )
    con.execute(
        f"CREATE VIEW team_seasons AS SELECT * FROM read_csv_auto('{_escape(TEAM_REGISTRY)}', SAMPLE_SIZE=-1, HEADER=TRUE)"
    )
...
def _connection() -> duckdb.DuckDBPyConnection:
    ...
    return _cached_connection(*signature)
...
def team_fixtures(season: str, team: str, limit: int = 100) -> ResearchResult:
    ...
    rows = con.execute(
        ...
        SELECT
            ...
            TRY_CAST(f.away_score AS INTEGER) AS away_score,
            CASE
                WHEN TRY_CAST(f.home_score AS INTEGER) IS NULL OR TRY_CAST(f.away_score AS INTEGER) IS NULL THEN NULL
                WHEN CAST(f.home_team_id AS VARCHAR) = s.local_team_id THEN TRY_CAST(f.home_score AS INTEGER)
                ELSE TRY_CAST(f.away_score AS INTEGER)
            END AS goals_for,
            CASE
                WHEN TRY_CAST(f.home_score AS INTEGER) IS NULL OR TRY_CAST(f.away_score AS INTEGER) IS NULL THEN NULL
                WHEN CAST(f.home_team_id AS VARCHAR) = s.local_team_id THEN TRY_CAST(f.away_score AS INTEGER)
                ELSE TRY_CAST(f.home_score AS INTEGER)
            END AS goals_against,
            CASE
                WHEN TRY_CAST(f.home_score AS INTEGER) IS NULL OR TRY_CAST(f.away_score AS INTEGER) IS NULL THEN 'UNPLAYED'
                WHEN CAST(f.home_team_id AS VARCHAR) = s.local_team_id AND TRY_CAST(f.home_score AS INTEGER) > TRY_CAST(f.away_score AS INTEGER) THEN 'W'
                WHEN CAST(f.away_team_id AS VARCHAR) = s.local_team_id AND TRY_CAST(f.away_score AS INTEGER) > TRY_CAST(f.home_score AS INTEGER) THEN 'W'
                WHEN TRY_CAST(f.home_score AS INTEGER) = TRY_CAST(f.away_score AS INTEGER) THEN 'D'
                ELSE 'L'
            END AS result
        FROM fixtures f
        JOIN selected s
