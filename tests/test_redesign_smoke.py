...
def test_redesign_python_files_compile():
    targets = [ROOT / "gui", ROOT / "poisson_model.py", ROOT / "player_research.py", ROOT / "query_api.py", ROOT / "query_lab.py"]
    failures: list[str] = []
...
def test_team_workspace_has_profile_and_stats_views():
    ...
    assert "render_team_workspace" in team_ui
    assert "_render_profile" in team_ui
    assert "_render_stats" in team_ui
    assert '"Profile", "Stats"' in team_ui
    ...
    assert "csv.read_csv" not in team_ui
    assert "read_csv(" not in team_ui
