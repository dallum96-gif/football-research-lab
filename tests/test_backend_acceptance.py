...
def test_backend_acceptance_all_public_families(monkeypatch):
    ...
    team_match = variable_resolver.resolve_variable(
        "goals", season="2024-25", fixture_id="1", family="team_match", team_id="t1"
    )
    player_season = variable_resolver.resolve_variable(
        "appearances", season="2024-25", family="player_season", player_id="p1"
    )
    squad = variable_resolver.resolve_variable(
        "player_name", season="2024-25", family="squad", player_id="p1"
    )
