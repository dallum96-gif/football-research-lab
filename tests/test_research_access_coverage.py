...
def test_player_match_coverage_is_reported_by_season(monkeypatch):
    ...
    assert "historical information availability" in result["temporal_note"]


def test_coverage_rejects_empty_season_list():
    try:
        research_access.coverage(variable="successfulDribbles", family="player_match", seasons=[])
    except research_access.ResearchAccessError as exc:
        assert "at least one season" in str(exc)
    else:
        raise AssertionError("coverage() must reject an empty season list")
