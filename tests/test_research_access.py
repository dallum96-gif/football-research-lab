...
def test_validate_core_request():
    request = research_access.ResearchRequest(
        variable="successfulDribbles", season="2024-25", family="player_match", fixture_id="1", player_id="p1")
...
def test_validate_fpl_request_uses_registry_path():
    request = research_access.ResearchRequest(variable="elements[].assists", season="2024-25", family="fpl", player_id="123", gameweek="1")
    result = research_access.validate(request)
    assert result["valid"] is True
...
def test_query_preserves_governed_result_semantics(monkeypatch):
    request = research_access.ResearchRequest(variable="successfulDribbles", season="2024-25", family="player_match", fixture_id="1", player_id="p1")
    monkeypatch.setattr(research_access, "resolve_variable", lambda **kwargs: {"results":[{"value":4}],"coverage":{"source_rows":1,"matched_rows":1},"population":{"requested":1,"observed":1},"source_rows":1,"temporal_note":"as_of=2024-25","limitations":["illustrative fixture scope"],"provenance":{"source_field":"successfulDribbles"}})
