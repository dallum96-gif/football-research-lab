...
def main():
    ...
    for row in rows:
        for entity in ("TEAM", "FIXTURE"):
            status = n(row.get(f"{entity.lower()}_route_verification"))
    ...
    by = Counter((n(r.get("target_entity")), n(r.get("grain"))) for r in frontier)
    print("\nGRAIN")
    for (e, g), c in sorted(by.items(), key=lambda x: (x[0][0], -x[1], x[0][1])):
        print(f"{e:8} {c:5} grain={g or '<blank>'}")

    resources = Counter((n(r.get("target_entity")), n(r.get("resource"))) for r in frontier)
    print("\nRESOURCE")
    for (e, resource), c in sorted(resources.items(), key=lambda x: (x[0][0], -x[1], x[0][1])):
        print(f"{e:8} {c:5} {resource or '<blank>'}")

    bases = Counter((n(r.get("target_entity")), n(r.get("route_evidence_basis"))) for r in frontier)
    print("\nEVIDENCE BASIS")
    for (e, basis), c in sorted(bases.items(), key=lambda x: (x[0][0], -x[1], x[0][1])):
        print(f"{e:8} {c:5} {basis or '<blank>'}")

    fields = list(dict.fromkeys(cols + [
        "target_entity", "verification_status", "route_evidence_basis"
    ]))
