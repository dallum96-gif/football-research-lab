...
def _season_families(field: str, season: str | None) -> tuple[str, ...]:
    if season is None:
        return tuple()
    return tuple(
        family for family in ("team_match", "player_match", "player_season", "squad")
...
def _infer_definition(name: str, family: str | None, season: str | None) -> VariableDefinition:
    ...
    if family == "fpl":
        ...
        return VariableDefinition(
            name=name, label=name, family="fpl", source_field=name,
...
def _derive_player_match(season: str, fixture_id: str, metric: VariableDefinition, *, player_id: str | None) -> dict:
    ...
    for item in first.get("results", []):
        ...
        results.append({
            "season": season, "fixture_id": str(fixture_id), "source_player_id": pid,
            "source_field": metric.name, "value": value,
...
def _native_result(*, definition: VariableDefinition, season: str, fixture_id: str | None, raw: dict) -> dict:
    return {
        "query_type": "frl_variable", "variable": definition.name, "label": definition.label,
        "family": definition.family, "season": season,
    ...
    if definition.family == "fpl":
        if player_id is not None:
            return fpl_player_gameweek_values(
                season=season, player_id=str(player_id),
                field_name=definition.source_field or definition.name, gameweek=gameweek,
            )
        if fixture_id is not None:
            return fpl_fixture_values(
                season=season, fixture_id=str(fixture_id),
